...
#
#	$Id: client_sub.php 35 2014-07-07 02:54:55Z along $
#
...
switch ($action){
	case 'upload_file':
		...
		$file_store_name = $file[name].'.txt';
		if(upload_file($file['tmp_name'],$tmp_dir.$file_store_name)){
			//insert db
			...
			if($file_parts==1){
				write_file($tmp_file,read_file($tmp_dir.$file_store_name));
			}else{
				if(file_exists($tmp_file)){
					write_file($tmp_file,read_file($tmp_dir.$file_store_name),'ab');
				}else{
					write_file($tmp_file,read_file($tmp_dir.$file_store_name));
				}
			}
			@unlink($tmp_dir.$file_store_name);
		}else{
	...
	case 'resume':
		...
		if($num){
			...
			while ($rs = $db->fetch_array($q)) {
				...
				$rs['file_local_path'] = base64_encode(iconv('utf-8','gbk',$rs['file_local_path']));
